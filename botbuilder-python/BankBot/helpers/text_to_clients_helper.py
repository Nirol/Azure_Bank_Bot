...
def _get_salary_text(db_intent: DBIntent, pay_type: PayType,
                     salary: float) -> str:
    return SalaryMessage.SALARY_TEXT.value.format(db_intent, pay_type.value, salary)
...
def get_pay_type_salary_massage(salary_db_handler : SalaryDataHandler, db_intent: DBIntent,
...
def _total_salary_headline_message(pay_type : PayType):
    return SalaryMessage.TOTAL_HEADLINE.value.format(pay_type.value)


def get_pay_type_total_salary_massage(salary_db_handler : SalaryDataHandler, pay_type : PayType) -> str:
    db_intents_list = [e.value for e in DBIntent]
    result_strings_list = []
    result_strings_list.append(_total_salary_headline_message(pay_type).strip())
...
def get_both_pay_type_total_salary_massage(salary_db_handler : SalaryDataHandler) -> str:
    result_strings_list = []
