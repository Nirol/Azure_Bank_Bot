# Copyright (c) Nir Gilad. All rights reserved.
...
class GeneralSalaryDialog(CancelAndHelpDialog):
    def __init__(self, salary_db_handler: SalaryDataHandler,
                 dialog_id: str = None, ):
        super(GeneralSalaryDialog, self).__init__(
            dialog_id or GeneralSalaryDialog.__name__)
    ...
    async def final_step(self,
        ...
        if general_salary_details and pay_type_user_input_text:
            general_salary_details.pay_type = text_from_clients_helper.fix_user_pay_type_input(
                pay_type_user_input_text)

        if general_salary_details:
            if intent_helper.is_specific_general_salary(
                    general_salary_details.salary_type):
                salary_msg = dialog_helper.get_salary_info(
                    self.salary_db_handler, general_salary_details)
            elif intent_helper.is_total_salary(
                    general_salary_details.salary_type):
